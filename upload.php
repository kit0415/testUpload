...
// if everything is ok, try to uploads file
       ...
       // $status =move_uploaded_file(basename($_FILES["fileToUpload"]["name"]), $target_dir);
       // echo "status is: ".$status;
        if (move_uploaded_file($_FILES["fileToUpload"]["name"], $target_file) && checkFileType($imageFileType) !==0 &&checkFileExists($target_file)==1) {
